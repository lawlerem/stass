...
#'   argument must be one those listed in get_stass_distributions().
...
covariance_from_formula<- function(x) {
  ...
  space<- function(covariance, nu) {
    ...
    match<- pmatch(
      covariance,
      get_stass_distributions("covariance"),
      duplicates.ok = TRUE
    )
    covar<- unname(get_stass_distributions("covariance")[match])
  ...
  the_terms<- terms(x, specials = c("time", "space", "effort"))
...
#' Retrieve an effort variable from a formula and a data.frame.
#'
#' @param x A formula object with an effort(...) function containing one
#'   entry.
#' @param data A data.frame containing the effort information.
#'
#' @return A data.frame with a single column for the effort.
#'
#' @noRd
effort_from_formula<- function(x, data, unique_vars = FALSE) {
  data<- data.frame(data)
  # Get out just the "effort" term from the formula (as a character string)
  the_terms<- terms(x, specials = c("time", "space", "effort"))
  the_idx<- attr(the_terms, "specials")$effort
  if( length(the_idx) == 0 ) {
    stop("Must supply an effort variable.")
    return(effort)
  } else if( length(the_idx) > 1 ) {
    stop("Multiple `effort` terms found in formula. Only one is allowed.")
  } else {}

  # [[2]] gets rid of effort()
  var_call<- as.list(attr(the_terms, "variables")[[the_idx + 1]][[2]])

  if( length(var_call) > 1 ) {
    stop("Only one effort variable allowed.")
  } else {}
  ...
  if( length(missing_vars) > 0 ) {
    stop(paste(
      "Effort variable not present in data: \n",
      paste(missing_vars, collapse=", ")
    ))
  } else {}


  effort<- data[, char_vars, drop = FALSE]
...
#' List of stass covariance functions
#'
#' Print a list of implemented covariance functions.
#'
...
#' @param which Unused
#'
#' @return A named character vector giving the implemented covariance functions
#'
#' @export
get_stass_distributions<- function(
    which = "covariance"
  ) {
...
mean_design_from_formula<- function(x, data, return = "model.matrix") {
  ...
  the_terms<- delete.response(
    terms(x, specials = c("time", "space", "effort"))
  )
...
time_from_formula<- function(x, data, return="vector") {
  ...
  time<- function(time_var, type = "ar1") {
    ...
    # Why is this not in get_stass_distributions?
    ...
    attr(time_col, "type")<- rep(type, length.out = 2)
    return(time_col)
  }
  ...
  if( length(the_idx) == 0 ) {
    ...
    attr(time_col, "type")<- rep("independent", 2)
    return(time_col)
  } else if( length(the_idx) > 1 ) {
