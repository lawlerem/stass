...
#'
#' @describeIn stass_fit Takes an unfitted stass object and performs
...
setMethod(
  ...
  # Update stass object based on parameter estimates
  tracing(object)<- tracing
  TMB_out(object)<- TMB_out
  object<- stass_update(object, TMB_out)
...
#
#' @export
#' @describeIn stass_simulate Simulate a new dataset from the model, with the
...
#' @export
#' @describeIn stass_predict Predict/forecast at the specific locations and
...
#'
#' @export
#' @describeIn stass_predict Predictions will be made for all raster cells
...
setMethod(
  ...
  if( length(covar_names) == 0 ) {
    ...
    pred<- stass_predict(x, prediction_points)
  ...
  var_stars<- lapply(seq_along(c("biomass", "effort", "suitability")), function(i) {
    pred_sf_list<- cbind(
      do.call(
        cbind,
        values(pred)[, , i]
  ...
  attr(pred_stars, "dimensions")[["variable"]]$values<- c("biomass", "effort", "suitability")
...
# #'
# #' @param x A stass object. If se = TRUE, should be a fitted stass object.
...
# #'
# #' @param x An stass object. If se = TRUE, should be a fitted stass object.
